...
export const createBooking = async (req, res) => {
    const { experienceId, slotId, userFullName, userEmail, numberOfPeople, finalPrice, // Must be calculated client-side (Checkout)
    promoCodeApplied, discountAmount } = req.body;
    ...
    try {
        ...
        const slotIndex = experience.slots.findIndex((s) => s._id.toString() === slotId);
        if (slotIndex === -1) {
            await session.abortTransaction();
